...
int main(int argc, char* argv[]) {
    ...
    if (argc < 2) {
	std::cerr << "usage: " << argv[0] << " <host> [data_size] [ttl]" <<
		std::endl;
	std::cerr << "\tdata_size can be up to " << MAX_PING_DATA_SIZE <<
		" bytes.  Default is " << DEFAULT_PACKET_SIZE << "." <<
		std::endl;
	std::cerr << "\tttl should be 255 or lower.  Default is " <<
		DEFAULT_TTL << "." << std::endl;
	return 1;
    }
    ...
    packet_size = std::max(sizeof (ICMPHeader),
	    std::min((unsigned int) MAX_PING_DATA_SIZE, (unsigned int) packet_size));
